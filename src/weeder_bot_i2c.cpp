#include "weeder_bot/weeder_bot_i2c.hpp"
...
namespace weeder_bot
{
	...
	void I2C::openfd()
	{
		if((fd = open(busfile, O_RDWR)) < 0) /*ROS_ERROR("Couldn't open I2C Bus %d [openfd():open %d]", _i2cbus, errno)*/;
		if(ioctl(fd, I2C_SLAVE, _i2caddr) < 0) /*ROS_ERROR("I2C slave %d failed [openfd():ioctl %d]", _i2caddr, errno)*/;
	}
}

// TODO: Code comments to be taken care
